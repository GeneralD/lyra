...
- **A test double that models time** — a probe that never answers, a stream that must hold its first line back, work that must really suspend — does not sleep either: the number would be part of no contract, and it either flakes under load or pads every run. A double that must never answer parks on `suspendUntilCancelled()` (`Tests/TestSupport/SuspendUntilCancelled.swift` — #353): on the passing path only the subject's cancel resumes it, and it returns `true`; on the failing path the helper's own guardrail (default 60 s, the suite limit) returns `false` and the double records nothing, so the assertion fails where it should. The guardrail is not redundant with `.timeLimit`: that cancel reaches only structured children, so a subject that regressed into running its probes on unstructured `Task { }`s would leave a cancel-only park waiting forever — a hang, not a failure. A double that must hold a value back until the test says so gates it on a `Collector<Void>` the test appends to, and records the moment the subject *asks* — an `AsyncStream(unfolding:)` runs its closure inside the consumer's `next()` — so the test can wait for the subject to be provably inside the call before it acts; a spin-wait the subject keeps private (`Task.yield()` until a resource is free) is observed by running that task on a test-owned `TaskExecutor` and waiting for its re-enqueue — a task that never suspends is enqueued exactly once, so a second enqueue *is* the suspension. A double that must suspend for real resumes a `CheckedContinuation` from a GCD queue instead of sleeping; the thread a sync→async bridge blocks on `semaphore.wait()` is out of reach of any in-process time limit, so for that suite the CI test job's `timeout-minutes` is the bound that turns a hang into a failure. (`LyricsCandidateWaveTests`, `MediaRemoteDataSourceImplTests`, `AsyncRunnableCommandTests` — #353)
