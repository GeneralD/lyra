...
**MediaRemoteDataSource via compiled helper**: Compiled binaries built by the main `lyra` build cannot access `MediaRemote.framework` (private framework) because of code-signature provenance. The helper Swift source (`Resources/media-remote-helper.swift`) is therefore compiled at runtime: on first launch (or after the bundled source SHA256 changes) `MediaRemoteDataSourceImpl.ensureExecutable()` runs `swiftc -O <src> -o ~/.cache/lyra/media-remote-helper` via `ProcessGateway.run`, then records the SHA in `media-remote-helper.swift.sha`. Subsequent launches skip the build and spawn the cached binary directly — eliminating the 1–2s `swift-frontend -interpret` cost on every cold start. The compiled binary runs as a persistent subprocess and streams JSON over a pipe, using `MRMediaRemoteRegisterForNowPlayingNotifications` for event-driven updates. `MediaRemoteDataSourceImpl(cacheHome:)` accepts an override directory so tests inject a temp dir instead of touching the real cache.
