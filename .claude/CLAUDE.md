...
**uta-net built-in lyrics tier (#317)**: `UtaNetLyricsDataSourceImpl` (a third `LyricsDataSource` DI key, `utaNetLyricsDataSource`) scrapes uta-net.com for Japanese plain lyrics and slots into `LyricsRepositoryImpl.fetchLyrics(candidates:)` as **Tier C**, demoting the user's `fallback_command` script to **Tier D** (it is the *user's* last resort, so built-in coverage runs first); the single-track path tries it after LRCLIB get/search. uta-net has no JSON API, so `UtaNetAPI` fetches raw HTML with a URLSession-direct client (not Papyrus — uta-net's capitalized query keys `Keyword`/`Aselect`/`Bselect` can't be derived from Swift parameter names) carrying a browser-like User-Agent: Cloudflare's bot check passes URLSession's TLS fingerprint but rejects curl's, and still wants a browser UA. `UtaNetHTMLParser` extracts the songlist table and `#kashi_area` div via Foundation `XMLDocument(.documentTidyHTML)` + XPath — zero new dependencies; a UTF-8 BOM is prepended before parsing because libxml2 doesn't sniff the HTML5 `<meta charset>` form and would garble Japanese as Latin-1. `get()` requires a non-empty artist and matches title+artist on a case-/width-/diacritic-folded letters+numbers key (uta-net lists same-titled covers and unrelated songs side by side); results carry `id: nil` (positive ids are LRCLIB's cache namespace — id-less rows get the negative synthetic id) and `duration: nil` (uta-net has none; `LyricsMatchValidator` then skips its duration check). `search()` materializes only the top row since each result costs an extra lyrics-page fetch. `UtaNetHealthCheck` probes the top page with the same UA.
