...
**Network session strategy — ephemeral session per call (#318)**: The API DataSources (LRCLIB lyrics, MusicBrainz, OpenAI-compatible LLM) must NOT hold a process-lifetime `URLSession`. In a long-running daemon, pooled connections go silently stale after sleep/wake or network changes, and with every request error swallowed by `try?` the only visible symptom was "lyrics never appear until `lyra restart`" (#318). Each call now builds a `URLSessionConfiguration.ephemeral` session — request timeout 10 s for LRCLIB/MusicBrainz, 60 s for the LLM (matches the old `URLSession.shared` default; local LLMs can take tens of seconds per completion) — through the DataSource's `apiFactory` closure, keeping the `init(api:)` test seam (it wraps a constant factory). The factory returns an `EphemeralSession<API>` wrapper class (`EphemeralSessionLRCLib` etc.) that owns the session and calls `finishTasksAndInvalidate()` in `deinit` — sessions must be explicitly invalidated to release their resources, so per-call sessions would otherwise accumulate in the long-lived daemon (the class is justified by that deinit-scoped cleanup). `LRCLibHealthCheck` already used this exact ephemeral+timeout construction; the daemon path now matches it. Failures are logged to stderr (`lyra: LRCLIB get failed: …`), except LRCLIB 404 which is the regular "no lyrics for this track" answer — so "no lyrics" and "fetch broken" stay distinguishable in the daemon log. Test gotcha: a per-call ephemeral session ignores `URLProtocol.registerClass` (that hook only applies to `URLSession.shared`), so production-wiring tests assert graceful degradation against a refused local port (`http://127.0.0.1:1`) instead of a global URL-protocol mock.
