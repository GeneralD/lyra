...
|---|---|---|
| **View** | `HeaderView`, `LyricsColumnView`, `LyricLineView`, `RippleView`, `SpectrumView`, `OverlayContentView`, `AppWindow`, `ConfigStatusOverlay` | Pure rendering. SwiftUI views get data from Presenters via `@ObservedObject`. `AppWindow` (NSWindow subclass) in Views module. `ConfigStatusOverlay` renders the destabilized-geodesic error indicator for config hot-reload failures (#41) |
...
| **Interactor** | `TrackInteractor`, `WallpaperInteractor`, `ScreenInteractor`, `SpectrumInteractor`, `ConfigInteractor` | Business logic. Abstractions in Domain, implementations in dedicated modules. TrackInteractor uses Combine hot stream. `ConfigInteractor` debounces file-watch events and republishes `ConfigUseCase.reload()` outcomes (#41) |
...
**Config hot-reload (#41)**: The daemon no longer needs a restart to pick up `config.toml` edits. `ConfigUseCaseImpl` replaces its former `lazy`-frozen load with a mutable `OSAllocatedUnfairLock<AppStyle>` store plus `reload() -> ConfigReloadOutcome` (Entity: `.updated(AppStyle)` / `.invalid(ConfigReloadFailure)`). On every failure path — an unreadable file, a decode error, or a file that exists but validates to defaults (the atomic-save half-write case) — the store is left untouched and `.invalid` carries the reason, so a bad edit can never regress the running style; only a genuinely absent config file legitimately resolves to `.updated(defaults)`. Watching follows the `ProcessGateway`/`AudioTapGateway` gateway shape: `ConfigWatchGateway` (Domain contract, `Sources/Domain/Misc/ConfigWatchGateway.swift`) is implemented by the new `FileWatchGateway` Support module via `DispatchSource.makeFileSystemObjectSource`, watching the config's **parent directory** rather than the file itself — editors save atomically (write-to-temp + rename), which invalidates a file-level fd but leaves the directory fd observing the rename. The new `ConfigInteractor` module (`ConfigInteractorImpl`) debounces watch events 150 ms (injected `ContinuousClock`, coalescing rapid edits and rename bursts) before calling `reload()`, then fans the outcome out over two Combine publishers: `appStyleChanges` (a `Void` ping on `.updated`) and `invalidConfig` (a `CurrentValueSubject<ConfigReloadFailure?, Never>` replaying the latest state). Because a daemon user watching the overlay has no terminal to read a stderr line from, the invalid state is surfaced graphically instead of logged: `ConfigStatusPresenter` (Presenters) exposes `invalidConfig` as `@Published`, and `ConfigStatusOverlay` (Views) renders a corner-anchored, amber, deliberately "destabilized" geodesic sphere (`DestabilizedGeodesicIndicator`, reusing `GeodesicGeometry.edges` from the gold `GeodesicLoadingIndicator` but with a deterministic missing-strut/jitter mask so it is never mistaken for the loading state) — zero-idle-cost the same way as `WallpaperLoadingOverlay` (#252). `AppRouter` wires `ConfigInteractor.start()/stop()` and `ConfigStatusPresenter.start()/stop()` alongside the other Presenters. **PR1 scope**: this ships the watch → debounce → reload → notify pipeline and the invalid-config indicator, plus making the lyrics `[lyrics] fallback_command`/`timeout_ms` hot-reloadable (`CustomScriptLyricsDataSourceImpl.get()` now re-reads config on every call instead of baking values in at init). **PR2 scope (Presenter re-render)**: `HeaderPresenter` and `LyricsPresenter` now consume the `appStyleChanges` ping via a `start()`/`applyStyle()` split — `start()` wires every subscription exactly once (including the `appStyleChanges` sink), and `applyStyle()` idempotently re-reads the owning Interactor's live config computed props (`textLayout`/`artworkStyle`/`decodeEffectConfig`) into `@Published` style state, so font/color/size/artwork (Header) and lyric/highlight styling (Lyrics) reflect a `config.toml` edit with no restart. The subscription is never torn down and rebuilt — the structural fix that killed the old push-model's double-subscription/`.share()`-replay/monitor-leak failure modes. Two carve-outs stay out of `applyStyle()` on purpose: (1) `DecodeEffectState` bakes `duration`/`charsets` as `let` at init, so `applyStyle()` itself never touches `titleEffect`/`artistEffect` — instead Header rebuilds them (`makeTitleEffect()`/`makeArtistEffect()`) at each reveal/processing boundary, strictly *after* the existing dedup guard confirms a new animation is starting, so decode *timing* config lands on the next reveal without interrupting an in-flight scramble (Codex review follow-up, PR2); (2) Header keeps a private `isAIProcessing` flag so an `applyStyle()` mid-AI-resolve preserves the #57 processing color instead of snapping back to the configured title color. **Still out of scope (follow-up PRs)**: re-rendering Wallpaper (source swap) and Screen (re-selection) on the same `appStyleChanges` ping.
