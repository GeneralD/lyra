...
/// Pure spectrum bar geometry (#23): every method is a pure mapping of
/// (size, style) → rects / points / alignment / paths, so it is unit-tested
/// directly against a `SpectrumGeometry` instance without a live
/// `GraphicsContext`. The Canvas drawing that consumes it lives in
/// `SpectrumRenderer`; the SwiftUI view struct in `SpectrumView`. The View and
/// the renderer each hold an instance rather than reaching into a global
/// namespace.
struct SpectrumGeometry {
    /// The visible bars of the current frame — those long enough to draw —
    /// laid out at a fixed `barWidth` thickness and `barSpacing` gap (cava
    /// style) and centered along the track. Each bar grows along the axis
    /// perpendicular to its anchoring edge: up/down for `bottom`/`top`,
    /// left↔right for the horizontal placements. Bars below half a point are
    /// dropped so the path carries no invisible slivers.
    func barRects(
        ...
        // Track = the edge-parallel axis the bars are distributed along;
        // growth = the perpendicular axis a bar extends along with its level.
        ...
        // An explicit radius is clamped to the bar half-thickness (a rounded
        // rect can't curve more than that); `nil` derives the cava-style default.
        let barCornerRadius =
    ...
    /// Growth-axis depth of the strip resolved from the placement's
    /// `SpectrumStyle` fields (a thin adapter over the core `stripDepth`).
    ...
    /// Growth-axis extent of the bar strip: `heightRatio` of the axis (the
    /// overlay height for vertical placements, the width for horizontal), then
    /// clamped into the optional `[minHeight, maxHeight]` point range (CSS
    ...
    func alignment(for placement: SpectrumPlacement) -> Alignment {
        switch placement {
    ...
    /// Horizontal placements rotate the bars into columns growing sideways;
    /// the vertical ones grow the bars up or down.
