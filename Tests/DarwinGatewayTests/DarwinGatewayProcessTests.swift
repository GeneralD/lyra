...
struct DarwinGatewayProcessTests {
    ...
    @Test("runInteractiveShell returns child termination status")
    func runInteractiveShellReturnsTerminationStatus() {
        let status = gateway.runInteractiveShell("exit 7")
        #expect(status == 7)
    }
