...
@Suite("FileWatchGateway", .timeLimit(.minutes(1)))
struct FileWatchGatewayTests {
    ...
    func firesOnWrite() async throws {
        ...
        let fired = Collector<Void>()
        let token = FileWatchGateway().watch(directory: dir.path) { fired.append(()) }
        #expect(token != nil)
        defer { token?.stop() }

        // `watch(...)` resumes the DispatchSource before returning the token,
        // so the event source is already armed — write immediately and await
        // the callback's own signal for delivery.
    ...
    func fileWatchFiresOnInPlaceWrite() async throws {
        ...
        let fired = Collector<Void>()
        let token = FileWatchGateway().watch(file: file.path) { fired.append(()) }
        #expect(token != nil)
        defer { token?.stop() }
