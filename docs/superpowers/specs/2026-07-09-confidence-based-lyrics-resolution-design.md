...
- `timeout_ms`(デフォルト5000ms)を超えたらプロセスをkillし、その候補は「マッチなし」として次候補に進む。killはSIGTERM→(500ms猶予後もrunningなら)対象pidへSIGKILLとエスカレーションし、SIGTERMを無視するスクリプトが居残らないようにする(プロセスグループではなく直接の子pidのみを対象とし、lyra本体を巻き込まない)。`timeout_ms`はユーザー入力なので、`Int`変換前に有限値1ms〜1時間へクランプする(`Int(Double)`はNaN/±inf/範囲外でトラップし、デーモンをクラッシュさせ得るため)。
- 候補は`MetadataRepositoryImpl`側で(title, artist, duration)の同一性で重複除去してから渡すため、raw/LLM/MusicBrainz/regexで同一の候補が複数回スクリプト起動されることはない。durationが同一性に参加するのは、LRCLIBの完全一致APIと`LyricsMatchValidator`がdurationで判別するため — 同名で長さの異なるMusicBrainz recordingは正当に別候補として残す。キーは文字列連結ではなく`Hashable`構造体(センチネル文字の衝突を構造的に排除)。
...
## レビュー反映(ラウンド2)— キャッシュ整合性の強化

PR #309のレビューで判明した、キャッシュ層の整合性ギャップへの対応:

- **MusicBrainzキャッシュは全候補recordingを保存する。** 従来はクエリごとに先頭1件のみ保存していたが、歌詞フローはどのMusicBrainz候補の下にでもヒットをキャッシュし得るため、キャッシュヒットが候補集合を先頭1件に切り詰めると、後続候補の下に書かれた歌詞エントリが2回目以降の再生で到達不能になる(1回目は歌詞が出て2回目から消える)。`MetadataDataStore<[MusicBrainzMetadata]>`に再パラメータ化し、v5マイグレーションで`musicbrainz_cache`のUNIQUE(query_title, query_artist)制約を外して1候補=1行で保存する(書き込みは同一キーの既存行を削除してから挿入)。
- **歌詞キャッシュは読み出し時にも`LyricsMatchValidator`で再検証する。** 検証導入前(pre-#308)に書かれた汚染エントリ(無関係な歌詞が候補キーの下にある)が、検証済みTier A/B/Cの実行を永久に短絡させるのを防ぐ。検証に落ちたエントリはスキップされ、後続のTierが正しいマッチで上書きする。
- **`displayAdjusted`はtitleとartistを独立に補完する。** Tier Cスクリプトは`artist_name`を省略でき、その場合に従来はtitleだけマッチ候補由来・artistは生データフォールバックという混在表示になっていた。欠けているフィールドだけをマッチ候補の値で埋める。
- **`[lyrics]`/`[ai]`は実行時は寛容、healthcheckは厳格。** 実行時デコード(`AppConfig.init`)は`try?`のまま維持し、任意機能セクションの形状ミスがビジュアル設定全体を道連れにしないようにする一方、`tryDecode()`(healthcheckのconfig検証経路)は両セクションを厳格に再デコードし、形状ミスを「検証エラー」として報告する — 機能が黙って無効化される事態をhealthcheckで検出可能にする。
